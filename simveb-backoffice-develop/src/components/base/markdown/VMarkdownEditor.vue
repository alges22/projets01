<script setup lang="ts">
	...
	type VMarkdownEditorToolbar = Record<string, VMarkdownEditorCommandAction | VMarkdownEditorCommandGroup>;
	...
	function triggerAction(action: VMarkdownEditorAction | ((ctx: VMarkdownEditorContext) => void | Promise<void>)) {
		...
		<VFlex justify-content="space-between" class="toolbar">
			<VFlexItem class="toolbar-mode">
				<VButtons addons>
					...
					<VAction dark="2" :disabled="!value" :active="mode === 'preview'" @click="mode = 'preview'">
						<VIcon icon="eye" />
						<span>Preview</span>
					</VAction>
				</VButtons>
			...
			<VFlexItem class="toolbar-actions">
				...
				<VButtons v-if="mode === 'write'" addons>
					<div v-for="(command, key) in props.toolbar" :key="key" class="toolbar-item">
						...
						<VDropdown v-else class="toolbar-dropdown" :class="[command.vertical && 'is-vertical']">

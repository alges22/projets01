<script setup lang="ts">
	...
	const { demand, url, formData: createData } = storeToRefs(duplicateStore);
	...
	onMounted(() => {
		if (!demand.value.id) {
			duplicateStore.fetchDemand(props.demandId).then(() => {
		...
		if (!createData.value?.services || !createData.value?.staff) {
			duplicateStore.loadTreatmentCreateData();
		}
	});
	...
	const assignToService = () => {
		...
		duplicateStore
			...
			.then((data) => {
				...
				duplicateStore.getDemand(demand.value.id).then(() => {
					treatment.value = demand.value.active_treatment;
				});
...
<template>
	<div class="columns tab-details-inner">
			...
			<VCard class="side-card">
				...
				<div v-else>
					...
					<div class="columns is-flex is-justify-content-flex-end mt-4">
						<VButton
							v-if="can('affect-to-service-card-duplicate')"
							color="primary"
							raised
					...
					<div class="columns is-flex is-justify-content-flex-end mt-4">
						<VButton
							v-if="
								...
								can('assign-to-staff-card-duplicate')
							"
							color="primary"
							raised
		...
		<VModal
			v-if="can('assign-to-staff-card-duplicate')"

<script setup>
	...
	const validate = () => {
		...
		client({
			...
			.then((response) => {
				notyf.success("Demande de levée de dage validée avec succès");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				client({
					...
					.then((response) => {
						notyf.success("Demande de levée de gage rejetée avec succès");
					...
					<div class="card-inner is-size-6-5">
						...
						<div class="columns">
							...
							<div class="column is-flex is-justify-content-space-between">
								<div>
									<VTag
										:label="pledgeLift.status_label"
										curved
										:color="statusColor(pledgeLift.status)"
									/>
								</div>
							</div>
						</div>
						<div class="columns">
							...
							<div class="column is-flex is-justify-content-space-between">
								<div>
									<span>{{ dayjs(pledgeLift?.created_at).format("DD/MM/YYYY H:m") }}</span>
								</div>
							</div>
						</div>
						...
						<div class="columns">
							...
							<div class="column">
								<span class="">{{
									pledgeLift.pledge?.financial_institution?.name ??
									pledgeLift.pledge?.institution_emitted.name
								}}</span>
							</div>
						</div>

						<div class="columns" v-if="pledgeLift.pledge?.financial_institution">
							<div class="column is-one-quarter">
								<span class="has-text-weight-semibold"> Concessionnaire </span>
							</div>
							<div class="column">
								<span class="">{{ pledgeLift.pledge?.institution_emitted.name }}</span>
							</div>
						</div>
					</div>
					...
					<div class="mt-8 flex justify-end">
						<VButton
							v-if="
								...
								pledgeLift.active_treatment.status === statuses.JUSTICE_VALIDATED
							"
							color="danger"
							raised
							size="medium"
				...
				<VCard class="side-card">
					<div class="card-inner is-size-6-5">
						<div class="columns">
							...
							<div class="column">
								<span> {{ pledgeLift.pledge?.vehicle_owner.identity.firstname }}</span>
							</div>
						</div>
						<div class="columns">
							<div class="column is-one-quarter">
							...
							<div class="column">
								<span> {{ pledgeLift.pledge?.vehicle_owner.identity.lastname }}</span>
							</div>
						</div>
						<div class="columns">
							<div class="column is-one-quarter">
							...
							<div class="column">
								<span> {{ pledgeLift.pledge?.vehicle_owner.identity.npi }}</span>
							</div>
						</div>
						<div class="columns">
							<div class="column is-one-quarter">
							...
							<div class="column">
								<span> {{ pledgeLift.pledge?.vehicle_owner.identity.telephone }}</span>
							</div>
						</div>
					</div>
				</VCard>

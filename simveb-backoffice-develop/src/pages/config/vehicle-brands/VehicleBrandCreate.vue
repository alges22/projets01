		...
		if (itemId) {
			crudStore.fetchRow(itemId).then((res) => {
				item.value = {
					...
					native_country: res.native_country,
				};
			});
		}
	});
	...
	const submit = () => {
		if (itemId) {
			crudStore.updateRow(itemId, item.value).then(() => {
				...
				router.push({ name: "vehicle_brands" });
			});
		} else {
		...
		<template #form-body>
			<div class="columns is-centered pt-2">
				<div class="column is-8">
					...
					<VField label="Pays de provenance" horizontal>
						<VControl fullwidth :errors="errors.native_country || []">
							<VInput
								v-model="item.native_country"
								name="native_country"

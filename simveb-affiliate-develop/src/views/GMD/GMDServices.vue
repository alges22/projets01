<template>
	...
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div v-if="can('store-gmd-vehicle')" class="col-span-12 md:col-span-6 bg-white rounded-md shadow-md">
			<CallToActionCard
			...
			>
				<div class="w-24 h-24 lg:w-16 lg:h-16 lg:mr-4 bg-primary rounded-full p-2 flex-center text-white">
					<i class="fa-light fa-car text-4xl"></i>
				</div>
			</CallToActionCard>
		</div>
		<div v-if="can('import-gmd-vehicle')" class="col-span-12 md:col-span-6 bg-white rounded-md shadow-md">
			<CallToActionCard
				button-text="Commencer"
	...
	<div class="dashboard-card !rounded-none mt-12">
		...
		>
			...
			<template #author="{ item }">{{ item.author?.identity.fullName }}</template>
			...
			<template #actions="{ item }">
				<router-link
					v-if="can('show-gmd-vehicle')"
					:to="{ name: 'gmd-vehicle', params: { id: item.id } }"
	...
	<VehicleRegistrationForm
		v-if="hasOnePermissions(['store-gmd-vehicle', 'update-gmd-vehicle'])"
		url="gmd-vehicles"
		...
		switch (active.value) {
			default:
				return [
					...
					{ key: "author", title: "Auteur", sortable: false },

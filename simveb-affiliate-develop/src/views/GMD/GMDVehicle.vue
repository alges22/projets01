<template>
	<div class="intro-y flex flex-col sm:flex-row items-center mt-4 justify-between">
		...
		<div class="w-full sm:w-auto flex mt-4 sm:mt-0 ml-auto">
			<button
				v-if="can('update-gmd-vehicle') && !data?.validated_at && !data?.rejected_at"
			...
			<template
				v-if="
					hasOnePermissions(['validate-gmd-vehicle', 'reject-gmd-vehicle']) &&
					!data?.validated_at &&
					!data.rejected_at
				"
			>
		...
		<div class="col-span-12 grid grid-cols-12 gap-6">
			...
			<div class="col-span-12 lg:col-span-6 mx-2 lg:mx-9 intro-y">
				...
				<div class="flex items-center pb-2 mt-2 border-b">
					...
					<a
						v-if="data.file"
						:href="data.file.path.path"
						download
						class="font-bold text-primary w-1/2 text-lg leading-5"
					>
...
<script setup>
	...
	const validateVehicle = async () => {
		Swal.fire({
			title: "Souhaitez-vous valider ce véhicule?",
			...
			if (result.isConfirmed) {
				url.value = "validate-gmd-vehicles";
				await vehicleStore.validateVehicle({ gmd_vehicles: [data.value.id] }).then(() => {

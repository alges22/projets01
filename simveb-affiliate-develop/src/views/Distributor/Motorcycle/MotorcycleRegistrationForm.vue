	...
	<Modal size="modal-lg" is-form :show="open" @submit="submit" @hidden="$emit('close')">
		...
		<ModalBody>
			<div class="flex flex-col justify-between">
				<template v-if="formType === 'add'">
					...
					<div class="my-4">
						<TextInputGroup
							v-model="vehicle.npi"
							name="npi"
							label="Entrer le npi du l'acheteur"
							placeholder="VN1324718265422NVV"
							add-class="w-full"
							:disabled="formLoading"
							:errors="errors.npi || []"
				...
				<template v-else>
					...
					<div class="mt-4">
						<a :href="urlFile" class="font-bold underline text-primary" download> Cliquez ici </a>
		...
		<ModalFooter class="">
			<div class="flex align-center justify-end mb-2">
				<BasicButton class="btn-primary w-full" type="submit" :loading="formLoading">
...
<script setup>
	...
	const { vehicle, formLoading, errors } = storeToRefs(useMotorcycleStore());
	...
	const submit = () => {
		if (formType.value === "add") {
			if (props.update) {
				...
				motorcycleStore.createVehicle(vehicle.value).then(() => {
					emit("close");
					motorcycleStore.fetchVehicles({});
					successModalOpen.value = true;
				});
			}
		} else {
			motorcycleStore.createVehicle(vehicle.value, true).then(() => {
				emit("close");
				successModalOpen.value = true;
			});
		}
	...
	onMounted(async () => {
		//await motorcycleStore.loadCreateData();
		motorcycleStore.getFilePath().then((response) => {
			urlFile.value = response;
		});
	});

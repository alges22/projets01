		...
		dom(node)
			...
			.each(function (key, el) {
				...
				if (key === props.selectedIndex) {
					const tab = tailwind.Tab.getOrCreateInstance(dom(el).find(".nav-linked")[0]);
					tab.show();
					dom(tabPanes).removeAttr("style");
				}

				const navLink = dom(el).find(".nav-linked")[0];
	...
	setup(props, { slots, attrs, emit }) {
		return () =>
			h(
				"li",
				{
					class: `nav-item ${props.fullWidth ? "flex-1" : ""} ${props.listClass}`,
					role: "presentation",
				},
				[
					h(

<script lang="ts" setup>
    ...
    const store = usePlateDuplicate()

    const loading = ref(false)

    const userStore = useUserStore()

    const {car: selectedCar, saving, front_plate, back_plate} = storeToRefs(store);
    ...
    const schema = yup.object({
        ...
        old_plate_id: yup
            .string()
            .required("Veuillez sélectionner la plaque à remplacer"),
    ...
    const onSubmit = (values) => {
        store.addToCart(id, user.value.identity.npi, values).then(() => {
            store.nextStep();
        }).catch((error) => {
            $awn.alert(error);
        });
                            ...
                            <span class="rounded-md bg-gray-200 text-gray-500 font-bold px-4 py-2">
                                ...
                                <div class="w-1/5 flex justify-end">
                                    <input
                                        ...
                                        type="radio"
                                        @change="store.fetchPlates(car.id)"
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-8 text-left mt-8">
                            <div
                                ...
                                @click="selectedPlate = back_plate.id"
                            >
                            <span>
                                Plaque avant
                            </span>
                            ...
                            <div
                                v-if="front_plate"
                                class="shadow rounded p-8 border-2 flex cursor-pointer w-full"
                                @click="selectedPlate = front_plate.id"
                            >
                            <span>
                                Plaque arrière
                            </span>
                                ...
                                <input
                                    ...
                                    type="radio"
                                    name="old_plate_id"
                                    :value="front_plate.id"
                                    v-model="selectedPlate"
                                />
                            </div>
                        </div>
                        <ErrorMessage class="form-invalid" name="old_plate_id" />
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-8 text-left mt-8">
                        ...
                        <div>
                            ...
                            <div class="shadow rounded p-8 border-2 flex cursor-pointer w-full"
                                ...
                                <span> La plaque est perdue </span>

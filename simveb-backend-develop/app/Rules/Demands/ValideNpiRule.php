...
class ValideNpiRule implements ValidationRule
{
    /**
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        if (strlen($value) != 10) {
            $fail(__('Le champ :attribute doit contenir 10 chiffres.'));
        }

        try {
            $checkNpiResponse = (new IdentityService)->showByNpi($value)->response()->getData(true)['data'];
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error($e);
            throw new UnexceptedErrorException;
        }

        if (!isset($checkNpiResponse['npi'])) {

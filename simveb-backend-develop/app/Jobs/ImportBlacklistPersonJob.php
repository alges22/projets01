...
use App\Imports\BlacklistPersonImport;
...
class ImportBlacklistPersonJob implements ShouldQueue
{
    ...
    {
        $import = new BlacklistPersonImport($this->authorProfile);
        $import->import(public_path($this->filepath));

        $hasFailure = isset($import->failures()[0]);

        $failureFilePath = $hasFailure ? (new HandleExcelErrorsService)->HandleExcelErrors($import, public_path($this->filepath), 'blacklist_persons') : '';

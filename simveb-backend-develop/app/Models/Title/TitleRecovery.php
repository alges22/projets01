...
class TitleRecovery extends Model implements CanFilterContract
{
    use HasUuids,
        HasFactory,
    ...
    protected static function boot()
    {
        ...
        static::creating(function ($model) {
            $model->reference = generateReference('RT');
        });
    }
    ...
    {
        return $this->belongsTo(TitleDeposit::class, 'deposit_id');
    }

    public function generateCertificate(bool $stream = false, string $filename = null): Response|string|null
    {
        $filename = $filename ?:
            Str::snake(class_basename($this)) . '_certificate_' . now()->timestamp . '_' . Str::random(3) . '.pdf';

        $output = GeneratePdfService::generatePDF(
            view: 'certificates.title-recovery',
            data: [
                'titleRecovery' => $this->load(TitleRecovery::relations())
            ],
            filename: $filename,
